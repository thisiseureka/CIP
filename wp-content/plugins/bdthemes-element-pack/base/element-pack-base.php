...
if (!class_exists("Element_Pack_Base")) {
    class Element_Pack_Base
    {
        ...
        {
            // For multisite installations, use the main site URL for license validation
            // This allows a single license to work across all sites in the network
            if (is_multisite()) {
            ...
            // For single site installations, use the regular site URL
        ...
        public static function get_multisite_info()
        {
            ...
            if ($info['is_multisite']) {
                ...
                // License uses main site URL for validation
                $info['license_domain'] = $info['main_site_url'];
            } else {
                // Single site
                if (function_exists("site_url")) {
                    $info['current_site_url'] = site_url();
        ...
        /**
         * Check if we're on a subsite and license is activated on main site
         * 
         * @return array
         */
        public static function get_subsite_license_status()
        {
            $status = array(
                'is_subsite' => false,
                'is_main_site_licensed' => false,
                'main_site_url' => '',
                'message' => '',
                'debug_info' => array()
            );
            ...
            if (is_multisite()) {
                // Check if this is NOT the main site
                $status['is_subsite'] = !is_main_site();
                $status['debug_info']['is_multisite'] = true;
                $status['debug_info']['is_main_site'] = is_main_site();
                
                if ($status['is_subsite']) {
                    // Get main site URL
                    if (function_exists("network_site_url")) {
                        $status['main_site_url'] = network_site_url();
                    } elseif (function_exists("network_home_url")) {
                        $status['main_site_url'] = network_home_url();
                    }
                    
                    // Check if license is activated (since we use main site domain for validation,
                    // the license check will already be using the main site's license)
                    $license_info = self::get_register_info();
                    $status['debug_info']['license_info_exists'] = !empty($license_info);
                    if ($license_info) {
                        $status['debug_info']['license_is_valid'] = !empty($license_info->is_valid);
                        $status['debug_info']['license_key'] = !empty($license_info->license_key) ? substr($license_info->license_key, 0, 8) . '...' : 'not_set';
                        $status['debug_info']['license_title'] = !empty($license_info->license_title) ? $license_info->license_title : 'not_set';
                    }
                    $status['debug_info']['main_site_id'] = get_main_site_id();
                    $status['debug_info']['current_blog_id'] = get_current_blog_id();
                    $status['is_main_site_licensed'] = !empty($license_info) && !empty($license_info->is_valid);
                    
                    if ($status['is_main_site_licensed']) {
                        $status['message'] = sprintf(
                            __('License is already activated on the main site (%s). No need to activate it again here.', 'bdthemes-element-pack'),
                            $status['main_site_url']
                        );
                    }
                }
            } else {
                $status['debug_info']['is_multisite'] = false;
            }
        ...
        public static function check_wp_plugin($purchase_key, $email, &$error = "", &$response_obj = null, $plugin_base_file = "")
        {
            ...
            if (is_multisite() && !is_main_site()) {
                $subsite_status = self::get_subsite_license_status();
                if ($subsite_status['is_subsite'] && $subsite_status['is_main_site_licensed']) {
                    // Get license info from main site for response_obj
                    $license_info = self::get_register_info();
                    if ($license_info) {
                        $response_obj = clone $license_info;
                        if (isset($response_obj->next_request)) {
        ...
        public static function get_register_info()
        {
            ...
            // For multisite subsites, try to get license info from main site
            if (is_multisite() && !is_main_site()) {
                // Switch to main site to get license data
                $main_site_id = get_main_site_id();
                $current_site_id = get_current_blog_id();
                
                if ($main_site_id && $main_site_id != $current_site_id) {
                    switch_to_blog($main_site_id);
                    
                    // Create a temporary instance to get the license data
                    $temp_obj = new self(BDTEP__FILE__);
                    $license_info = $temp_obj->get_old_wp_response();
