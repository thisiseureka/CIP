...
class Breadcrumbs extends Module_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			'change_text',
			[
				'label' => esc_html__('Change Text', 'bdthemes-element-pack') . BDTEP_NC,
		...
		$this->add_control(
			'home_page_text',
			[
				'label' => esc_html__('Home Page', 'bdthemes-element-pack'),
				'type' => Controls_Manager::TEXT,
				'default' => (is_home() || is_front_page()) ? esc_html__('Home', 'bdthemes-element-pack') : get_bloginfo('name'),
				'dynamic' => ['active' => true],
				'condition' => [
					'change_text' => 'yes',
				],
	...
	private function _breadcrumbs($align, $home_icon) {
		...
		if ($settings['change_text']) {
			$home = isset($settings['home_page_text']) ? $settings['home_page_text'] : '';
		} else {
			if (is_home() || is_front_page()) {
				$home        = __('Home', 'bdthemes-element-pack');
			} else {
