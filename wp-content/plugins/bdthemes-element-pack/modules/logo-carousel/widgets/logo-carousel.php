...
class Logo_Carousel extends Module_Base {
	...
	public function get_style_depends() {
		if ( $this->ep_is_edit_mode() ) {
			...
			return [ 'swiper', 'ep-font', 'ep-logo-carousel', 'tippy' ];
		}
	}

	public function get_script_depends() {
		if ( $this->ep_is_edit_mode() ) {
			return [ 'swiper', 'popper', 'tippyjs', 'ep-scripts' ];
		} else {
			return [ 'swiper', 'popper', 'tippyjs', 'ep-logo-carousel' ];
		}
	}
	...
	public function render_navigation( $settings ) {
		...
		?>
		<div
			class="bdt-position-z-index bdt-position-<?php echo esc_attr( $arrows_position ); ?> <?php echo esc_attr( $hide_arrow_on_mobile ); ?>">
	...
	public function render_loop_item( $settings ) {
		...
		foreach ( $settings['logo_list'] as $index => $item ) :
			...
			$link_key = 'carousel_item' . $index . '_link';
			...
			if ( $item['link']['url'] ) {
				$this->add_render_attribute( $link_key, 'class', 'bdt-logo-carousel-link' );
			...
			if ( $item['name'] and $item['description'] and $item['logo_tooltip'] ) {
				...
				if ( $item['logo_tooltip_placement'] ) {
					$this->add_render_attribute( $repeater_key, 'data-tippy-placement', esc_attr( $item['logo_tooltip_placement'] ), true );
				}
				...
				<figure <?php $this->print_render_attribute_string( 'image-wrap' ); ?>>
					<?php if ( $image ) :
						...
						printf( '<img class="bdt-logo-carousel-img elementor-animation-%s" src="%s" alt="%s">',
							...
							esc_url( BDTEP_ASSETS_URL . 'images/gallery/item-' . ( $index + 1 ) . '.svg' ),
							esc_attr( $image_alt )
						);
					endif; ?>
