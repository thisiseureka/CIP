...
class Module extends Element_Pack_Module_Base {
	...
	public function element_pack_woocommerce_show_product_images() {
		...
		<div class="ep-images">
			...
			if ( has_post_thumbnail() ) {
				...
				echo wp_kses_post(apply_filters( 'woocommerce_single_product_image_html', sprintf( '<a href="%s" itemprop="image" class="woocommerce-main-image zoom" title="%s" data-rel="prettyPhoto' . $gallery . '">%s</a>', $props['url'], $props['caption'], $image ), $post->ID ));
			} else {
				echo apply_filters( 'woocommerce_placeholder_img', sprintf(
					'<img src="%s" alt="%s" width="%s" class="woocommerce-placeholder wp-post-image" height="%s" />',
					esc_url( wc_placeholder_img_src() ),
					esc_html__( 'Placeholder', 'bdthemes-element-pack' ),
					esc_attr($placeholder_image_data[0]),
					esc_attr($placeholder_image_data[1])
				), $post->ID );
			}
			...
			while ( $ajaxposts->have_posts() ) :
				...
				if ( isset( $ajaxSettings['show_badge'] ) && ( 'yes' === $ajaxSettings['show_badge'] ) ) :
					if ( ! $product->is_in_stock() ) :
						...
						$markup .= apply_filters( 'woocommerce_product_is_in_stock', '<span class="bdt-onsale">' . esc_html__( 'Out of Stock!', 'bdthemes-element-pack' ) . '</span>', $post, $product );
						$markup .= '</div>';
					elseif ( $product->is_on_sale() ) :
						$markup .= '<div class="bdt-badge bdt-position-top-left bdt-position-small">';
						$markup .= apply_filters( 'woocommerce_sale_flash', '<span class="bdt-onsale">' . esc_html__( 'Sale!', 'bdthemes-element-pack' ) . '</span>', $post, $product );
						$markup .= '</div>';
					endif;
				endif;
	...
	public function __render_add_to_cart() {
		...
		echo wp_kses_post(apply_filters(
			...
			sprintf(
				'<a href="%s" data-quantity="%s" class="%s" %s>%s <i class="button-icon ep-icon-arrow-right"></i></a>',
