...
class WC_Slider extends Module_Base {
    ...
    {
        return ['slider', 'woocommerce', 'wc', 'product', 'carousel', 'wc slider'];
    }
    ...
    public function get_style_depends()
    {
        if ($this->ep_is_edit_mode()) {
            ...
            return ['swiper', 'ep-font', 'ep-wc-slider'];
        }
    }
    ...
    {
        if ($this->ep_is_edit_mode()) {
            return ['swiper', 'imagesloaded', 'ep-woocommerce', 'ep-scripts'];
        } else {
            return ['swiper', 'imagesloaded', 'ep-woocommerce', 'ep-wc-slider'];
        }
    }
    ...
    protected function register_controls()
    {
        ...
        $this->add_control(
            ...
            [
                ...
                'description' => esc_html__( 'Slider ratio to width and height, such as 16:9', 'bdthemes-element-pack' ),
                'condition' => [ 
                    'enable_height!' => 'yes'
                ]
            ]
        ...
        $this->add_control(
			'enable_height',
			[ 
				'label'   => esc_html__( 'Enable Responsive Height', 'bdthemes-element-pack' ) . BDTEP_NC,
        ...
        $this->add_responsive_control(
			...
			[
				...
				'default'   => 'left',
				'selectors' => [
					'{{WRAPPER}} .bdt-wc-slider .bdt-badge' => '{{VALUE}}',
				],
        ...
        $this->add_responsive_control(
            ...
            [
                ...
                'size_units' => ['px', 'rem', 'em', '%'],
                'selectors' => [
                    '{{WRAPPER}} .bdt-wc-slider .bdt-dotnav li a' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
                ],
                'condition' => [
                    'navigation!' => ['arrows', 'none'],
                ],
		...
		$this->add_control(
			...
			[
				'label' => esc_html__( 'Transition Duration', 'bdthemes-element-pack' ) . ' (s)',
				'type' => Controls_Manager::SLIDER,
				'range' => [
					'px' => [
						'min' => 0,
...
$bdt_counter = 0;
    ...
    public function render_header()
    {
        ...
        $ratio = ( ! empty( $settings['slider_size_ratio']['width'] ) && ! empty( $settings['slider_size_ratio']['height'] ) ) ? $settings['slider_size_ratio']['width'] . ":" . $settings['slider_size_ratio']['height'] : '1920:768';
                ...
                'slider_settings' => [
                    'data-bdt-slideshow' => [
                        wp_json_encode( [ 
							...
							"ratio"             => $ratio,
							"min-height"        => ( ! empty( $settings['slider_min_height']['size'] ) && $ratio !== false ) ? $settings['slider_min_height']['size'] : false,
        ...
        $slider_fullscreen = '';
		if ($settings['slider_fullscreen'] && ! $settings['enable_height']) {
			$slider_fullscreen = 'bdt-height-viewport="offset-top: true"';
		}
    ...
    public function render()
    {
        ...
        while ($wp_query->have_posts()): $wp_query->the_post();
            ...
            ?>
	                                    <?php echo wp_kses_post(apply_filters('woocommerce_product_is_in_stock', '<span class="bdt-onsale">' . esc_html__('Out of Stock!', 'bdthemes-element-pack') . '</span>', $post, $product)); ?>
	                                </div>
	                            <?php elseif ($settings['show_badge'] and $product->is_on_sale()): ?>
                                <div class="bdt-badge bdt-position-top-left bdt-position-small">
                                    <?php //woocommerce_show_product_loop_sale_flash();
        ?>
                                    <?php echo wp_kses_post(apply_filters('woocommerce_sale_flash', '<span class="bdt-onsale">' . esc_html__('Sale!', 'bdthemes-element-pack') . '</span>', $post, $product)); ?>
                                </div>
                            <?php endif;?>
