...
class Testimonial_Slider extends Module_Base {
	...
	public function get_style_depends() {
		if ( $this->ep_is_edit_mode() ) {
			...
			return [ 'swiper', 'ep-font', 'ep-testimonial-slider' ];
		}
	}
	public function get_script_depends() {
		if ( $this->ep_is_edit_mode() ) {
			...
			return [ 'swiper', 'ep-testimonial-slider' ];
		}
	}
	...
	public function register_controls() {
		...
		$this->add_control(
			'show_text',
			[ 
				'label'     => esc_html__( 'Text', 'bdthemes-element-pack' ) . BDTEP_NC,
				'type'      => Controls_Manager::SWITCHER,
				'default'   => 'yes',
				'separator' => 'before',
			]
	...
	public function render_excerpt() {
		...
		if ($settings['text_read_more_toggle'] == 'yes') {
			if ( isset( $settings['text_limit'] ) && ! empty( $settings['text_limit'] ) ) {
				...
				$this->add_render_attribute( 'read-text', 'data-read-more', wp_json_encode(
					[ 
	...
	public function render_meta( $element_key ) {
		...
		if ( $settings['title'] or $settings['company_name'] or $settings['rating'] ) :		
			$company_name = get_post_meta( get_the_ID(), 'bdthemes_tm_company_name', true );
		?>
			<div class="bdt-testimonial-meta <?php echo ( $settings['meta_multi_line'] ) ? 'bdt-meta-multi-line' : ''; ?>">
				<?php if ( $settings['title'] ) : ?>
					<div class="bdt-testimonial-title">
						<?php echo esc_attr(get_the_title()); ?><?php if ( $settings['show_comma'] ) {
							echo ( ( $settings['title'] ) and ( $settings['company_name'] ) and $company_name ) ? ', ' : '';
						} ?>
					</div>
