...
class Marker extends Module_Base {
	...
	public function get_style_depends() {
		if ( $this->ep_is_edit_mode() ) {
			...
			return [ 'ep-font', 'ep-marker', 'tippy' ];
		}
	}
		...
		if ( $has_caption ) : ?>
			...
			<div <?php $this->print_render_attribute_string( 'wrapper' ); ?>>
				...
				foreach ( $settings['markers'] as $index => $marker ) {
					...
					$this->add_render_attribute( $element_key, 'data-tippy-content', htmlspecialchars( $marker['marker_title'], ENT_QUOTES, 'UTF-8' ), true );

					if ( 'lightbox' == $marker['link_to'] ) {
						$this->add_render_attribute( $element_key, 'data-elementor-open-lightbox', 'no', true );
						$this->add_render_attribute( $element_key, 'data-caption="' . htmlspecialchars( $marker['marker_title'] ) . '"' );
