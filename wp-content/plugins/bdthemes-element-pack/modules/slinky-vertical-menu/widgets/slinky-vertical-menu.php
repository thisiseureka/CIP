...
class Slinky_Vertical_Menu extends Module_Base {
	...
	protected function register_controls() {
		...
		$this->add_control(
			'sticky_offset',
			[
				'label'   => esc_html__('Sticky Offset', 'bdthemes-element-pack') . BDTEP_NC,
				'type'    => Controls_Manager::SLIDER,
				'default' => [
					'size' => 0,
				],
		...
		$this->add_control(
			'sticky_on_scroll_up',
			[
				'label'        => esc_html__('Sticky on Scroll Up', 'bdthemes-element-pack') . BDTEP_NC,
				'type'         => Controls_Manager::SWITCHER,
		...
		$this->add_control(
			'sticky_edge',
			[
				'label'       => __('Scrolling Area', 'bdthemes-element-pack') . BDTEP_NC,
	...
	protected function render() {
		...
		if ( 'yes' == $settings['show_sticky'] ) {
			$this->add_render_attribute( 'slinky_vertical_menu', 'data-bdt-sticky', "media: 1024;" );

			if ($settings['sticky_offset']['size']) {
				$this->add_render_attribute('slinky_vertical_menu', 'data-bdt-sticky', 'offset: ' . $settings['sticky_offset']['size'] . ';');
			}
			if ($settings['sticky_on_scroll_up']) {
				$this->add_render_attribute('slinky_vertical_menu', 'data-bdt-sticky', 'show-on-up: true; animation: bdt-animation-slide-top;');
			}
			if ($settings['sticky_edge']) {
				$this->add_render_attribute('slinky_vertical_menu', 'data-bdt-sticky', 'bottom: ' . esc_attr($settings['sticky_edge']) . ';');
			}
		}
	...
	protected function dynamic_menu() {
		...
		$nav_menu_args = array(
			'fallback_cb'    => false,
			'container'      => false,
            'items_wrap'     => '<ul id="%1$s" class="slinky-vertical-menu %2$s">%3$s</ul>',
			'menu_id'        => $id,
			'menu_class'     => '',
