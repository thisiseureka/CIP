...
class wpdatatable extends Module_Base {
	...
	protected function get_table_list() {
		if ( shortcode_exists( "wpdatatable" ) ) {
			...
			$returnTables   = [];
			$returnTables[] = __( 'Choose a table', 'bdthemes-element-pack' );
	...
	public function render() {
		...
		if ( $settings['table_id'] ) {
			...
			echo '<div class="bdt-alert bdt-alert-warning">' . esc_html__( 'Please select a table from setting!', 'bdthemes-element-pack' ) . '</div>';
		}
	}
	...
	function wp_data_table( $id, $atts = null, $content = null ) {
		...
		if ( empty( $tableData->content ) ) {
			return __( 'wpDataTable with provided ID not found!', 'bdthemes-element-pack' );
		}
