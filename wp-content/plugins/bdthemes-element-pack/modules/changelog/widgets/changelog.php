...
class Changelog extends Module_Base {
	...
	public function get_title() {
		return __( 'Changelog', 'bdthemes-element-pack' );
	}
	...
	protected function register_controls() {
		...
		$this->add_control(
			'chnagelog_file_url',
			[
				'label' => __( 'Changelog File', 'bdthemes-element-pack' ),
				'type' => Controls_Manager::TEXT,
				'placeholder' => __( 'http://example.com/changelog.md', 'bdthemes-element-pack' ),
				'label_block' => true,
			]
		);
	...
	protected function render() {
        ...
        echo '<div class="bdt-ep-changelog" data-bdt-scrollspy="target: > ul > li; cls: bdt-animation-slide-right-small; delay: 50">';
    		...
    		if ( false === $data ) {
	            ...
	            if ( is_array( $response ) && ! is_wp_error( $response ) && 404 !== $response['response']['code'] ) {
					...
					if (Plugin::instance()->editor->is_edit_mode()) {
						$data = sprintf('<div class="bdt-alert-warning bdt-padding-small" bdt-alert>%s</div>', esc_html__('Response error file not found!', 'bdthemes-element-pack'));
					}
	            }
