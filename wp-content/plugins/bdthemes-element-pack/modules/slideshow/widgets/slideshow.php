...
class Slideshow extends Module_Base {
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			'text_stroke_width',
			[ 
				'label'     => esc_html__( 'Text Stroke Width', 'bdthemes-element-pack' ),
				'type'      => Controls_Manager::SLIDER,
				'selectors' => [ 
	...
	protected function render_header() {
		...
		if ($settings['slideshow_fullscreen'] && ! $settings['enable_height']) {
	...
	protected function render_item_image( $image ) {
		...
		$image_src = $image_src ? $image_src[0] : $image['image']['url'];

		if ( $image_src ) :
			echo '<img src="' . esc_url( $image_src ) . '" alt="' . esc_html( get_the_title() ) . '" class="bdt-cover" data-bdt-cover="">';
		endif;
