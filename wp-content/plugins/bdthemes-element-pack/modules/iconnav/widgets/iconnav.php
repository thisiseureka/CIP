...
class Iconnav extends Module_Base {
	...
	public function render_loop_iconnav_list( $list, $element_key ) {
		...
		if ( 'show_as_tooltip' == $settings['menu_text'] ) {
			...
			$this->add_render_attribute( 'iconnav-item', 'data-tippy-content', htmlspecialchars( $list["iconnav_title"], ENT_QUOTES, 'UTF-8' ), true );
	...
	private function offcanvas( $settings ) {
		...
		if ( 1 == $settings['navbar_level'] ) {
			$nav_class = 'bdt-nav bdt-nav-default bdt-nav-no-parent-icon';
		} else {
