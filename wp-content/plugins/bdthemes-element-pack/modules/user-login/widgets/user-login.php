...
class User_Login extends Module_Base {
	...
	protected function register_controls() {
		...
		$this->start_controls_tabs( 'default_' );
		...
		$this->start_controls_section(
			'section_style_logout_button_default',
			[ 
				'label'     => esc_html__( 'Logout Button', 'bdthemes-element-pack' ),
				'tab'       => Controls_Manager::TAB_STYLE,
				'condition' => [ 
		...
		$this->start_controls_tabs( 'default_tabs_logout_button_style' );
		$this->start_controls_tab(
			'default_tab_logout_button_normal',
			[ 
				'label' => esc_html__( 'Normal', 'bdthemes-element-pack' ),
			]
		);
		$this->add_control(
			'logout_button_default_text_color',
			[ 
				'label'     => esc_html__( 'Text Color', 'bdthemes-element-pack' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [ 
					'{{WRAPPER}} .bdt-user-login .bdt-user-logged-out a.bdt-button' => 'color: {{VALUE}} !important;',
				],
			]
		);
		$this->add_group_control(
			...
			[ 
				...
				'exclude'   => [ 'image' ],
				'selector'  => '{{WRAPPER}} .bdt-user-login .bdt-user-logged-out a.bdt-button',
			]
		);
		$this->add_group_control(
			Group_Control_Border::get_type(),
		...
		$this->add_responsive_control(
			'logout_button_default_border_radius',
			[ 
				'label'      => esc_html__( 'Border Radius', 'bdthemes-element-pack' ),
				'type'       => Controls_Manager::DIMENSIONS,
				'size_units' => [ 'px', '%' ],
		...
		$this->end_controls_tab();
		$this->start_controls_tab(
			'default_tab_logout_button_hover',
			[ 
				'label' => esc_html__( 'Hover', 'bdthemes-element-pack' ),
			]
		);
		$this->add_control(
			'logout_button_default_hover_text_color',
			[ 
				'label'     => esc_html__( 'Text Color', 'bdthemes-element-pack' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [ 
					'{{WRAPPER}} .bdt-user-login .bdt-user-logged-out a.bdt-button:hover' => 'color: {{VALUE}} !important;',
				],
			]
		);
		$this->add_group_control(
			...
			[ 
				...
				'types'     => [ 'classic', 'gradient' ],
				'exclude'   => [ 'image' ],
				'selector'  => '{{WRAPPER}} .bdt-user-login .bdt-user-logged-out a.bdt-button:hover',
			]
		);
		$this->add_control(
			'logout_button_default_hover_border_color',
			[ 
				'label'     => esc_html__( 'Border Color', 'bdthemes-element-pack' ),
				'type'      => Controls_Manager::COLOR,
				'selectors' => [ 
					'{{WRAPPER}} .bdt-user-login .bdt-user-logged-out a.bdt-button:hover' => 'border-color: {{VALUE}};',
				],
				'condition' => [ 
						...
						<div class="bdt-social-login">
							...
							if ( ( ! $fb_secret or ! $fb_app_id ) && 'yes' == $settings['show_facebook_login'] ) :
								...
								<div class="bdt-alert-warning" data-bdt-alert>
									...
									<p>
										<?php
										/* translators: %1$s is the missing API setting name */
										echo sprintf( esc_html__( 'Ops! %1$s Missing. Please add them from: Element Pack Settings > API Settings > Social Login Access. ', 'bdthemes-element-pack' ), esc_html( $fb_message ) ); ?>
									</p>
								</div>
							...
							if ( ! $gl_app_id && 'yes' == $settings['show_google_login'] ) :
								...
								<div class="bdt-alert-warning" data-bdt-alert>
									...
									<p>
										<?php
										/* translators: %1$s is the missing API setting name */
										echo sprintf( esc_html__( 'Ops! %1$s Missing. Please add them from: Element Pack Settings > API Settings > Social Login Access. ', 'bdthemes-element-pack' ), esc_html( $google_app_message ) ); ?>
									</p>
								</div>
	...
	public function user_login_form() {
		...
		echo esc_attr( $id ); ?>" <?php
			...
			$this->print_render_attribute_string( 'field-group' ); ?>>
				...
				echo ('yes' == $settings['toggle_password']) ? '<div class="bdt-toggle-pass-wrapper"><i class="ep-icon-eye"></i></div>' : '';
				echo '</div>';
				?>
			</div>
