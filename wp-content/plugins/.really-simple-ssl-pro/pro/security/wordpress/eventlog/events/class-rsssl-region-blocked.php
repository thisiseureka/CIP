 ...
 */
class Rsssl_Region_Blocked extends Rsssl_Event_Log_Handler {
	/**
	 * Class constructor.
	 *
	...
	public static function handle_event( array $data = array(), $event_data = array() ): void {
		...
		$event['description'] = $_self->set_message( array( 'region_name' => $data['region_name'] ), $event['description'] );

		$event_data['iso2_code']        = '';
		$event_data['country_name'] = $data['region_name'];
	...
	protected function set_message( array $args, string $message ): string {
		return sprintf( __( $message, 'really-simple-ssl' ), $args['region_name'] );
	}
}
