...
class rsssl_scan {
	...
	public function get_notices_list($notices)
	{
		$notices['mixed_content_scan'] = array(
			'callback' => 'RSSSL()->scan->pro_scan_notice',
			'score' => 10,
			'output' => array(
				'has-ssl-scan-has-errors' => array(
					'msg' => __("The last scan was completed with errors. Only migrate if you are sure the found errors are not a problem for your site.", "really-simple-ssl"),
					'icon' => 'warning',
					'dismissible' => true,
				),
				'no-ssl-scan-has-errors' => array(
					'msg' => __("The last scan was completed with errors. Are you sure these issues don't impact your site?", "really-simple-ssl"),
					'icon' => 'warning',
					'dismissible' => true,
				),
			),
	...
	public function pro_scan_notice() {
		if ( !rsssl_get_option('site_has_ssl') ) {
			if ( $this->scan_completed_no_errors() == "ERRORS" ) {
				return 'has-ssl-scan-has-errors';
			} elseif ( ! RSSSL()->licensing->license_is_valid() ) {
   ...
   * Get last scan time
	...
	private function get_last_scan_time() {
		if ( $this->last_scan_time == __( "Never", "really-simple-ssl" ) ) {
			return $this->last_scan_time;
		}
		if ( ! empty( $this->last_scan_time ) ) {
			//$date = date(DateTime::RFC850);
			return sprintf(
			    _x( "%s at %s","'date' at 'time'", "really-simple-ssl" ),
	...
	public function run_scan() {
		...
		if ( $iteration == 1 ) {
			...
			update_option( 'rsssl_current_action', __( "Generating web page list", "really-simple-ssl" ), false );
			$this->save_results();
		}
		...
		if ( $iteration == 2 ) {
			...
			update_option( 'rsssl_current_action',
				sprintf( __( "Searching for js and css files and links to external resources in website, %d of %d",
					"really-simple-ssl" ), $current_queue,
		...
		if ( $iteration == 4 ) {
			...
			$in_queue    = $this->still_in_queue( 0 );
			update_option( 'rsssl_progress', $progress, false );
			update_option( 'rsssl_current_action', __( "Generating file list", "really-simple-ssl" ), false );
			$this->save_results();
		}
		...
		if ( $iteration == 6 ) {
			...
			$in_queue = $this->still_in_queue( 0 );
			update_option( 'rsssl_progress', $progress, false );
			update_option( 'rsssl_current_action', __( "Checking widgets for external URLs", "really-simple-ssl" ), false );
			$this->save_results();
		}
		...
		if ( $iteration == 8 ) {
			...
			update_option( 'rsssl_progress', $progress, false );
			update_option( 'rsssl_current_action', sprintf( __( "Checking which resources can't load over ssl, %s of %s", "really-simple-ssl" ), $current_queue, count( $this->external_resources ) ), false );
			$this->save_results();
		}
		...
		if ( $iteration == 14 ) {
			...
			update_option( 'rsssl_current_action', __( "Finished scan", "really-simple-ssl" ), false );
...
//		die;
		...
		if ( $handle = opendir( $path ) ) {
			while ( false !== ( $file = readdir( $handle ) ) ) {
				if ( $file != "." && $file != ".." ) {
					...
					//we also exclude backup files generated by really simple Security, rsssl-bkp-
   ...
   */
	...
	public function get_post_using_this_block( $post_id ) {
		...
		$sql = "select * from " . $wpdb->base_prefix
...
*/
	...
	public function get_file_type_description($file){
		if ( strpos( $file, "themes" ) !== false ) {
			return __( "theme file", "really-simple-ssl" );
		} elseif ( strpos( $file, "mu-plugins" ) !== false ) {
			return __( "mu plugin file", "really-simple-ssl" );
		} elseif ( strpos( $file, "plugins" ) !== false ) {
			return __( "plugin file", "really-simple-ssl" );
		} elseif ( strpos( $file, "uploads" ) !== false ) {
			return __( "uploads file, possibly generated by plugin or theme", "really-simple-ssl" );
		} elseif ( strpos( $file, "cache" ) !== false ) {
			return __( "cached file, deactivate cache to see the actual source", "really-simple-ssl" );
		} else {
			return __( "file", "really-simple-ssl" );
		}
	}
	...
	public function get() {
		...
		$copyright_fix_warning = [__( "Downloading files from other websites can cause serious copyright issues! It is always illegal to use images, files, or any copyright protected material on your own site without the consent of the copyrightholder. Please ask the copyrightholder for permission. Use this function at your own risk.", "really-simple-ssl" ),
		                         __( "This downloads the file from the domain without SSL, inserts it into WP media, and changes the URL to the new URL.", "really-simple-ssl" )];
		...
		foreach ( $this->files_with_blocked_resources as $file => $urls ) {
			...
			foreach ( $urls as $blocked_url ) {
				$results_id ++;
				$modal_explanation = [sprintf( __( "Blocked URL: %s", "really-simple-ssl" ), $blocked_url ), __( "You can edit the source file manually by pressing the edit button.", "really-simple-ssl" )];
				$data[] = [
					'id'               => $results_id,
					'ignored'          => $this->ignore($blocked_url),
					'type'             => 'blocked_url',
					'description'      => sprintf( __( "Mixed content in PHP file in %s", "really-simple-ssl" ), $item_title ),
		...
		foreach ( $this->css_js_with_mixed_content as $file => $mixed_resources ) {
			...
			foreach ( $mixed_resources as $blocked_url ) {
				...
				$modal_explanation = [sprintf( __( "Blocked URL: %s", "really-simple-ssl" ), $blocked_url )];
				$modal_explanation[] = $file_type === 'mu-plugins' ? __("Can be fixed manually by editing the respective mu-plugin file in the /wp-content/mu-plugins/ directory.", "really-simple-ssl" ) : __( "Can be fixed automatically by pressing the Fix button. If fixing fails, the source file can be edited manually by pressing the Edit button.", "really-simple-ssl" );
				$results_id ++;
				$data[] = [
					'id'               => $results_id,
					'ignored'          => $this->ignore($blocked_url),
					'description'      => sprintf(__("Mixed content in %s", "really-simple-ssl"), $item_title),
					'type'             => $type,
					'blocked_url'      => $blocked_url,
					'location'         => $this->get_path_to( $file_type, $file ),
					'details'    => [
		...
		foreach ( $external_css_js_with_mixed_content as $remote_url => $blocked_urls ) {
			foreach ( $blocked_urls as $blocked_url ) {
				$results_id ++;
				$modal_explanation = [__( "Cannot be fixed automatically, as the mixed content is coming from an external domain. Contact the owner of the domain to update the CSS/JS file", "really-simple-ssl" )];
				$modal_explanation[] = sprintf( __( "Mixed content resources: %s", "really-simple-ssl" ), $blocked_url );
				$data[] = [
					'id'               => $results_id,
					'ignored'          => $this->ignore($blocked_url),
		...
		foreach ( $this->posts_with_blocked_resources as $post_id ) {
			...
			foreach ( $blocked_urls as $blocked_url ) {
				...
				$results_id ++;
				$explanation = [sprintf( __( "Blocked URL: %s", "really-simple-ssl" ), $blocked_url ),
				                __( "Mixed content found in in a post. Can be fixed automatically by pressing the fix button. Pressing the edit button allows you to update the link in the post manually.", "really-simple-ssl" )];
				$data[] = [
					'id'               => $results_id,
					'ignored'          => $this->ignore($blocked_url),
					'type'             => 'posts',
					'description'      => sprintf(__( "Mixed content in post: %s", "really-simple-ssl" ), get_the_title( $post_id ) ),
					'blocked_url'      => $blocked_url,
		...
		foreach ( $this->postmeta_with_blocked_resources as $post_id ) {
			...
			foreach ( $blocked_urls as $meta_key => $blocked_url ) {
				...
				$description = $this->source_is_iframe( $post_id, $blocked_url, $meta_key ) ? __( "iFrame in the wp_postmeta database table", "really-simple-ssl" ) : __( "Mixed content in the postmeta table", "really-simple-ssl" );
				$modal_explanation = [sprintf( __( "Blocked URL: %s", "really-simple-ssl" ), $blocked_url ),
				                __( "Mixed content from a postmeta table in your database. Usually won't cause any mixed content on the front-end. Check the post if it causes mixed content. If so, the link can be replace directly in the database.", "really-simple-ssl" )];
				$data[] = [
					'id'               => $results_id,
					'ignored'          => $this->ignore($blocked_url),
					'type'             => 'postmeta',
		...
		foreach ( $this->widgets_with_blocked_resources as $widget_name ) {
			...
			foreach ( $blocked_urls as $blocked_url ) {
				...
				$file = sprintf( __("Widget area \"%s\"", "really-simple-ssl" ), $item_title);
				if (isset( $widget_data["title"]) && strlen( $widget_data["title"])>0) {
					$file .= ': '. $widget_data["title"];
				}
				$results_id++;
				$modal_explanation = [sprintf( __( "Blocked URL: %s", "really-simple-ssl" ), $blocked_url ), __( "Mixed content found in a widget. Press the edit link to edit the widget manually.", "really-simple-ssl" )];
				$data[] = [
					'id'               => $results_id,
					'ignored'          => $this->ignore($blocked_url),
					'type'             => 'widgets',
					'description'      => __( "Widget with mixed content", "really-simple-ssl" ).': '.$item_title,
					'blocked_url'      => $blocked_url,
					'location'         => $file,
					'details'   => [
						'title'       => __("Details", "really-simple-ssl"),
	...
	private function faker()
	{
		...
		$this->files_with_blocked_resources = [
			'test_file.php' => ['http://blockedresource.com', 'http://blockedresource2.com', 'http://blockedresource3.com'],
			'datatable_view_test.php' => ['http://blockedresource.com', 'http://blockedresource2.com', 'http://blockedresource3.com'],
		];

		$this->css_js_with_mixed_content = [
			'mixed_content_file.php' => ['https://securecontent.com', 'http://nonsecurecontent.com'],
		];

		$this->files_with_css_js = [
			'css_js_error_file.php' => ['http://errorresource1.com', 'http://errorresource2.com'],
		];

		$this->files_with_external_css_js = [
			'external_css_js_blocked_file.php' => ['http://blockedexternalresource1.com', 'http://blockedexternalresource2.com'],
		];

		$this->files_with_nonexistent_resources = [
			'nonexistent_file.php' => ['http://nonexistentresource.com'],
		];
...
//		delete_transient( 'rsssl_scan' );
		$this->scan_completed_no_errors = get_option( 'rsssl_scan_completed_no_errors', 'NEVER' );
		$this->last_scan_time = get_option( 'rsssl_last_scan_time', __( "Never", "really-simple-ssl" ) );
  ...
  */
   ...
   */
	...
	public function roll_back_modal() {
		$args = array(
			'title'    => __( "Roll back changes made to your files", "really-simple-ssl" ),
			'subtitle' => __( "Copyright warning!", "really-simple-ssl" ),
			'id'       => 'roll-back-modal',
			'content'  => array(
				__( "This will put the files back that were changed by the fix option in Really Simple Security Pro.",
					"really-simple-ssl" ),
			...
			'buttons'  => array(
				1 => array(
					'text'  => __( 'Restore files', 'really-simple-ssl' ),
					'id'    => 'start-roll-back',
